    ...
    def read_xplane_DATA(self):
      with closing(self.sock):
        ...
        while self.stop_program == False :
          ...
          self.delta_baffer = np.asarray([self.DATA_Xplane['roll'] , self.DATA_Xplane['pitch'] , self.DATA_Xplane['hding_true'],self.DATA_Xplane['alt_ftagl']])

          self.flight_data_for_integrate = self.flight_data_for_integrate.append(pd.DataFrame(\
              [[self.DATA_Xplane['roll'] , self.DATA_Xplane['pitch'] , self.DATA_Xplane['hding_true'] , self.DATA_Xplane['alt_ftagl'] , self.DATA_Xplane['real']]],\
              columns = ['roll','pitch','yaw','altitude','time']))
          #print(type(self.DATA_Xplane[42]))
